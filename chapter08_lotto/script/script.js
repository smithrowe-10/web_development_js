...
// script.js에 코드 검증하는 부분들 작성해줄것 => main 은 script2.js 확인

let list = ['abc', 123, true, false, 'ABC'];

list.sort;

console.log(list); // 결과값 ['abc', 123, true, false, 'ABC']

// 결과값을 확인 했을 때, 숫자 1이 가장 작고, 숫자 t가 가장 크다고 정리가 됩니다
// .sort() 함수의 기본 정렬 방식은 => 유니코드의 코드값을 기반으로 함

// 특징 : .sort()를 실행하게 되면 일단 모든 데이터를 str로 전환한 후에 크기를 비교하여 정렬하고, 다시 원래의 자료형으로 변환하여 제시 => 이상을 이유로 서로 다른 자료형을 가진 list가 정렬이 가능했습니다.

// 이상의 특징 때문에 1, 11, 2 와같은 방식으로의 정렬이 이루어집니다.
